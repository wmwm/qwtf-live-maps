# QWTF Live Map Repo

Community catalog + asset source for the maps actually played on QWTF Live.
Not an archive of every map ever made for QuakeWorld Team Fortress —
scoped to the current competitive rotation and what real matches show
people actually playing, cross-checked against [logs.qwtf.live](https://logs.qwtf.live/).

## Map status legend

- **loc status**: `author-in-map` (extracted from the map's own built-in location entities — highest confidence), `community` (hand-authored, imported from the wider QW scene), `heuristic-unreviewed` (auto-generated from spawn/flag/item positions, needs a playtester's eyes), `missing`.

## 2v2

| Map | Matches (logs.qwtf.live) | Assets | Loc status |
|---|---|---|---|
| `blitzkrieg2` | 171 | bsp+ent+tex | heuristic-unreviewed |
| `egypt3` | 125 | bsp+ent | heuristic-unreviewed |
| `genders2` | 63 | bsp+ent+tex | heuristic-unreviewed |
| `poop` | 52 | bsp+ent+tex | heuristic-unreviewed |
| `nbases_b1` | 47 | bsp | heuristic-unreviewed |
| `5speed5` | 34 | bsp+ent | heuristic-unreviewed |
| `dragongod2` | 33 | bsp | author-in-map |
| `rs_zz1` | 27 | bsp+ent | heuristic-unreviewed |
| `desperat` | 26 | bsp+ent+tex | heuristic-unreviewed |
| `preskool` | 25 | bsp+ent+tex | heuristic-unreviewed |
| `2farms` | 23 | bsp+ent | heuristic-unreviewed |
| `mountdoom` | 22 | bsp | author-in-map |
| `qu4d` | 20 | bsp+ent | heuristic-unreviewed |
| `etf_miniheros` | 17 | bsp+tex | heuristic-unreviewed |
| `ftactic1` | 17 | bsp+ent | heuristic-unreviewed |
| `optics6` | 17 | bsp | heuristic-unreviewed |
| `etf_egypt` | 14 | bsp+tex | heuristic-unreviewed |
| `2fort5r` | 13 | bsp | community |
| `castlemania_b2` | 12 | bsp | heuristic-unreviewed |
| `etf_xpress` | 12 | bsp+tex | heuristic-unreviewed |
| `1on1forts` | 10 | bsp | community |
| `lostfortress` | 10 | bsp | heuristic-unreviewed |
| `cyclonr` | 9 | bsp+tex | author-in-map |
| `egypt` | 9 | bsp | heuristic-unreviewed |
| `noad2` | 9 | bsp | heuristic-unreviewed |
| `wc3` | 9 | bsp | heuristic-unreviewed |
| `well6mini` | 9 | bsp | author-in-map |
| `disorder` | 8 | bsp | heuristic-unreviewed |
| `drgnfyr2` | 8 | bsp | heuristic-unreviewed |
| `ins3` | 8 | bsp | heuristic-unreviewed |
| `lotus_b4` | 8 | bsp | author-in-map |
| `alchemy` | 7 | bsp | heuristic-unreviewed |
| `substation_b2` | 7 | bsp | author-in-map |
| `fo_labs_b7` | 6 | bsp | heuristic-unreviewed |
| `h4rdcoremini` | 6 | **missing** | missing |
| `vertex1r` | 6 | bsp | heuristic-unreviewed |
| `r123` | 5 | bsp | heuristic-unreviewed |
| `8-1` | 4 | bsp | heuristic-unreviewed |
| `smap47` | 4 | bsp | heuristic-unreviewed |
| `sunnyramps` | 4 | bsp | heuristic-unreviewed |
| `spyder` | 3 | bsp | heuristic-unreviewed |
| `volcanic` | 3 | bsp | heuristic-unreviewed |
| `2machr` | 2 | bsp | heuristic-unreviewed |
| `marics_ctf1` | 2 | bsp | heuristic-unreviewed |
| `neenf` | 2 | bsp | heuristic-unreviewed |
| `utumno_b3` | 2 | bsp | heuristic-unreviewed |

## 3v3

| Map | Matches (logs.qwtf.live) | Assets | Loc status |
|---|---|---|---|
| `ff-phantomr` | 398 | bsp+tex | heuristic-unreviewed |
| `ff-destroy3` | 343 | bsp+tex | heuristic-unreviewed |
| `japanc` | 340 | bsp+tex | heuristic-unreviewed |
| `rs_zzr` | 320 | bsp | heuristic-unreviewed |
| `h5rdcore_b2` | 180 | bsp+ent+tex | heuristic-unreviewed |
| `blitzkrieg2` | 171 | bsp+ent+tex | heuristic-unreviewed |
| `egypt3` | 125 | bsp+ent | heuristic-unreviewed |
| `genders2` | 63 | bsp+ent+tex | heuristic-unreviewed |
| `ff-siege` | 60 | bsp+ent | heuristic-unreviewed |
| `nbases_b1` | 47 | bsp | heuristic-unreviewed |
| `dragongod2` | 33 | bsp | author-in-map |
| `3way2` | 26 | bsp | heuristic-unreviewed |
| `japk` | 26 | bsp+ent | heuristic-unreviewed |
| `middleskool` | 22 | bsp+ent | author-in-map |
| `god_well` | 13 | bsp+tex | heuristic-unreviewed |
| `nightwatch2` | 9 | bsp | author-in-map |
| `drgnfyr2` | 8 | bsp | heuristic-unreviewed |
| `haste` | 7 | bsp | heuristic-unreviewed |
| `cyanr` | 6 | bsp | heuristic-unreviewed |
| `minitf2k` | 6 | bsp | heuristic-unreviewed |
| `rmanor_b6` | 6 | bsp | author-in-map |
| `caverns` | 5 | bsp | heuristic-unreviewed |
| `42smooth` | 4 | bsp | heuristic-unreviewed |
| `beyonddarkness` | 4 | bsp | heuristic-unreviewed |
| `2night3_b1` | 3 | bsp | heuristic-unreviewed |
| `echo_b1` | 3 | bsp | author-in-map |
| `snowlake_b7` | 3 | bsp | author-in-map |
| `bastion` | 2 | bsp | heuristic-unreviewed |
| `shank76` | 2 | bsp | heuristic-unreviewed |
| `anaconda` | 1 | bsp | heuristic-unreviewed |
| `elusive` | 1 | **missing** | missing |
| `qbase1_b3` | 1 | bsp | author-in-map |
| `sanctuary_b9` | 1 | bsp | author-in-map |
| `spring_b4` | 1 | bsp | author-in-map |
| `vengeance2` | 1 | bsp+tex | author-in-map |
| `box4` | 0 | **missing** | missing |
| `nightshacksb5` | 0 | **missing** | missing |

## 4v4

| Map | Matches (logs.qwtf.live) | Assets | Loc status |
|---|---|---|---|
| `ff-phantomr` | 398 | bsp+tex | heuristic-unreviewed |
| `ff-schtop` | 355 | bsp+tex | heuristic-unreviewed |
| `ff-destroy3` | 343 | bsp+tex | heuristic-unreviewed |
| `japanc` | 340 | bsp+tex | heuristic-unreviewed |
| `rs_zzr` | 320 | bsp | heuristic-unreviewed |
| `openfirer` | 317 | bsp | heuristic-unreviewed |
| `ff-swoop` | 217 | bsp+ent+tex | heuristic-unreviewed |
| `h5rdcore_b2` | 180 | bsp+ent+tex | heuristic-unreviewed |
| `blitzkrieg2` | 171 | bsp+ent+tex | heuristic-unreviewed |
| `pineapple_b3` | 100 | bsp | heuristic-unreviewed |
| `turtler` | 87 | bsp+ent+tex | heuristic-unreviewed |
| `tfc-demolish_b2` | 59 | bsp | heuristic-unreviewed |
| `ff-shoop` | 34 | bsp | heuristic-unreviewed |
| `dragongod2` | 33 | bsp | author-in-map |
| `ff-mortality` | 26 | bsp+tex | author-in-map |
| `turbo` | 26 | bsp+ent | heuristic-unreviewed |
| `tf2day_b2` | 21 | bsp+tex | heuristic-unreviewed |
| `ff-aardvark` | 15 | bsp+ent | heuristic-unreviewed |
| `canal_b7` | 13 | bsp | author-in-map |
| `ff-2fort` | 13 | bsp+tex | author-in-map |
| `god_well` | 13 | bsp+tex | heuristic-unreviewed |
| `excel3` | 11 | bsp | author-in-map |
| `fo-spiderx` | 8 | bsp | author-in-map |
| `nbases_b2` | 7 | bsp | heuristic-unreviewed |
| `42smooth` | 4 | bsp | heuristic-unreviewed |
| `ff-ibex` | 1 | bsp | heuristic-unreviewed |
| `fracturex2` | 0 | **missing** | missing |

## 5v5

| Map | Matches (logs.qwtf.live) | Assets | Loc status |
|---|---|---|---|
| `ff-schtop` | 355 | bsp+tex | heuristic-unreviewed |
| `ff-destroy3` | 343 | bsp+tex | heuristic-unreviewed |
| `openfirer` | 317 | bsp | heuristic-unreviewed |
| `blitzkrieg2` | 171 | bsp+ent+tex | heuristic-unreviewed |
| `ff-mortality` | 26 | bsp+tex | author-in-map |
| `ff-aardvark` | 15 | bsp+ent | heuristic-unreviewed |
| `fo_smartbases_b2` | 11 | bsp | author-in-map |
| `fo-spiderx` | 8 | bsp | author-in-map |
| `etf-bigzag_b1` | 7 | bsp | heuristic-unreviewed |
| `ff-raiden` | 4 | bsp | heuristic-unreviewed |
| `impactr` | 2 | bsp | heuristic-unreviewed |
| `ff-ibex` | 1 | bsp | heuristic-unreviewed |
| `quartz_b1` | 1 | bsp | author-in-map |
| `picnics_b3` | 0 | bsp | author-in-map |

## Played but not in the static rotation list

Found via real match data on logs.qwtf.live, not the Discord `!maps` output — flagged here for a human call on whether the rotation list itself is stale.

- `castrum2` — 1 matches, assets: yes
- `fracturex` — 1 matches, assets: yes
- `wellgl1` — 2 matches, assets: yes
- `xpress3` — 5 matches, assets: yes

## Gaps — no local asset found

These are in scope but no `.bsp` exists anywhere in this workspace. Not invented, not silently dropped — need sourcing.

- `box4`
- `elusive`
- `fracturex2`
- `h4rdcoremini`
- `nightshacksb5`

